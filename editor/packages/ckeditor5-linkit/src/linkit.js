...
export default class Linkit extends Plugin {
  ...
  constructor(editor) {
    ...
    // Tried something here, does not work...
    editor.model.schema.extend( '$text', { allowAttributes: 'dataEntityType' } );
    editor.model.schema.extend( '$text', { allowAttributes: 'dataEntityUUID' } );
    editor.model.schema.extend( '$text', { allowAttributes: 'dataEntitySubstitution' } );
    ...
    /*editor.conversion.for( 'upcast' ).elementToAttribute( {
      ...
      model: {
        key: 'dataEntityType',
        value: viewElement => viewElement.getAttribute( 'data-entity-type' )
      }
    } );*/
    ...
    editor.conversion.for( 'upcast' ).elementToAttribute( {
      ...
      model: {
        key: 'dataEntityUUID',
        value: viewElement => viewElement.getAttribute( 'data-entity-uuid' ),
      }
    } );
    ...
    /*editor.conversion.for( 'upcast' ).elementToAttribute( {
      ...
      model: {
        key: 'dataEntitySubstitution',
        value: viewElement => viewElement.getAttribute( 'data-entity-substitution' )
      }
    } );*/

    /*editor.conversion.for( 'downcast' ).attributeToElement( {
      model: 'dataEntityType',
      view: ( dataEntityTypeValue, writer ) => {
        console.log('entity type:', dataEntityTypeValue);
        const attributes = {'data-entity-type': dataEntityTypeValue}
    ...
    editor.conversion.for( 'downcast' ).attributeToElement( {
      ...
      view: ( dataEntityUUIDValue, writer ) => {
        console.log('uuid:', dataEntityUUIDValue);
        const attributes = {'data-entity-uuid': dataEntityUUIDValue}
        const element = writer.createAttributeElement( 'a', attributes, { priority: 5 } );
        writer.setCustomProperty( 'link', true, element );
    ...
    /*editor.conversion.for( 'downcast' ).attributeToElement( {
      model: 'dataEntitySubstitution',
      view: ( dataEntitySubstitutionValue, writer ) => {
        const attributes = {'data-entity-substitution': dataEntitySubstitutionValue}
        const element = writer.createAttributeElement( 'a', attributes, { priority: 5 } );
        writer.setCustomProperty( 'link', true, element );
    ...
    return dispatcher => {
      dispatcher.on( 'attribute:linkHref', ( evt, data, conversionApi ) => {
        ...
        for ( const item of this._definitions ) {
          ...
          viewWriter.setAttribute('data-entity-uuid', 'test', viewElement);
          //console.log('new: ', item.attributes);
  ...
  _createLinkitButton() {
    ...
    button.on('execute', () => {
      ...
      this.requestInformation('select-link', {
        ...
        if (link.href !== null) {
          // Add a link decorator that will set the needed attributes for the
          // linkit filter.
          const automaticDecorators = new AutomaticDecorators();
          this.add( {
            id: 'linkitContent',
            mode: 'automatic',
            callback: url => url === link.href && link['data-entity-type'] && link['data-entity-uuid'] && link['data-entity-substitution'],
            attributes: {
              'data-entity-type': link['data-entity-type'],
              'data-entity-uuid': link['data-entity-uuid'],
              'data-entity-substitution': link['data-entity-substitution'],
            }
          } );
          console.log('Enable downcast for: ' , link.href);
          editor.conversion.for( 'downcast' ).add( this.getDispatcher());
